...
function Activities({ initialData, loginName, loginAttuid, isLogin }) {
  ...
  const updateData = async (id) => {
    ...
    setData(temp);
    setActivities(temp.issues);
  };

  if (isLogin) {
    let temp = data.members.filter((member) => {
      return member.attuid === loginAttuid;
    });
    if (temp.length > 0) isMember = true;
  }
  ...
  const [activities, setActivities] = useState(initialData.issues);
  useEffect(() => {
  ...
  const otherHandler = async () => {
    ...
    await fetch(`http://localhost:4000/team/${data.id}`, {
      method: "PATCH",
      body: JSON.stringify({ issues: [newActivity, ...activities] }),
      headers: { "Content-Type": "application/json" },
    });
    setModalText("Issue added successfully !");
    setShowModal(true);
