...
function Activities({ initialData, loginName, loginAttuid, isLogin }) {
  ...
  const [activities, setActivities] = useState(initialData.ideas);
  ...
  const otherHandler = async () => {
    ...
    await fetch(`http://localhost:4000/team/${data.id}`, {
      method: "PATCH",
      body: JSON.stringify({ ideas: [newActivity, ...activities] }),
      headers: { "Content-Type": "application/json" },
    });
    setModalText("Idea added successfully !");
    setShowModal(true);
